...
export default function CartTable({
  ...
  const subtotal = (shoppingCart) => {
    return shoppingCart.reduce(
      (subt, product) =>
        subt + products[findProduct(product.itemId)]?.price * product?.quantity,
      0
    );
  };
  ...
  return (
      <table className="checkoutTable" style={style}>
        ...
        {shoppingCart?.map((product) => (
          <tr>
            <td className="table-entry">
              {products[findProduct(product.itemId)]?.name}
            </td>
            <td className="table-entry">{product?.quantity}</td>
            <td className="table-entry">{`$${products[
              findProduct(product.itemId)
            ]?.price.toFixed(2)}`}</td>
            <td className="table-entry">{`$${(
              products[findProduct(product.itemId)]?.price * product?.quantity
            ).toFixed(2)}`}</td>
          </tr>
        ))}
        {
        ...
        <tr>
          ...
          <td></td>
          <td>{`$${(
            (subtotal(shoppingCart) * 8.75) / 100 +
